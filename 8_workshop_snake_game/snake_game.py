...
def end_screen():
    ...
    fill(255)
    textSize(64)
    text(config.GAME_OVER_MESSAGE, config.WINDOW_WIDTH / 2 - len(config.GAME_OVER_MESSAGE) * 15,
         config.WINDOW_HEIGHT / 2)
    if score.score > score.highscore:
        text(config.HIGH_SCORE_MESSAGE, config.WINDOW_WIDTH / 2 - len(config.HIGH_SCORE_MESSAGE) * 15,
             config.WINDOW_HEIGHT / 2 + 64)
